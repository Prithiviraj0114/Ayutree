...
def test_checkout_cod(s, cust_headers, stocked_cart):
    ...
    assert d["total"] > 0
    assert d.get("razorpay_order_id") is None
    # cart cleared
    cart = s.get(f"{API}/cart", headers=cust_headers).json()
    assert cart["items"] == []
    # appears in orders
...
def test_checkout_razorpay_simulated_and_verify(s, cust_headers, stocked_cart):
    ...
    assert r.status_code == 200, r.text
    d = r.json()
    assert d.get("razorpay_order_id") is not None
    ...
    import hmac
    import hashlib
    key_secret = "eDByyQMB3ZPUysxAFUFZzuu3"
    pay_id = f"pay_sim_{uuid.uuid4().hex[:8]}"
    msg = f"{d['razorpay_order_id']}|{pay_id}"
    signature = hmac.new(
    ...
    # verify
    vr = s.post(f"{API}/payments/razorpay/verify", json={
        "order_id": d["order_id"],
